...
export default function Modal({
  ...
  return (
    <AlertDialog defaultOpen={isOTP}>
      {!isOTP && (
        <AlertDialogTrigger className={showModalStyles}>{showModalText}</AlertDialogTrigger>
      )}
      <AlertDialogContent className="gap-8">
        {closeBtn && (
          <AlertDialogCancel className={closeBtnStyles}>
        ...
        {title ? (
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-foreground">{text}</AlertDialogDescription>
          </AlertDialogHeader>
        ) : null}
        {children}
        <AlertDialogFooter className={`${!isOTP ? "items-center sm:justify-center" : ""}`}>
          {btnRejectText?.length > 0 && (
            <AlertDialogCancel className={btnRejectStyles}>{btnRejectText}</AlertDialogCancel>
          )}
          {btnConfirmText?.length > 0 && (
            <AlertDialogAction className={btnConfirmStyles} onClick={onSubmit}>
              {btnConfirmText}
            </AlertDialogAction>
          )}
        </AlertDialogFooter>
